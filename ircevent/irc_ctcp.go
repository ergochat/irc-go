...
func (irc *Connection) setupCTCPCallbacks() {
	irc.AddCallback("CTCP_VERSION", func(e ircmsg.Message) {
		irc.SendRaw(fmt.Sprintf("NOTICE %s :\x01VERSION %s\x01", e.Name(), irc.Version))
	})

	irc.AddCallback("CTCP_USERINFO", func(e ircmsg.Message) {
		irc.SendRaw(fmt.Sprintf("NOTICE %s :\x01USERINFO %s\x01", e.Name(), irc.User))
	})

	irc.AddCallback("CTCP_CLIENTINFO", func(e ircmsg.Message) {
		irc.SendRaw(fmt.Sprintf("NOTICE %s :\x01CLIENTINFO PING VERSION TIME USERINFO CLIENTINFO\x01", e.Name()))
	})

	irc.AddCallback("CTCP_TIME", func(e ircmsg.Message) {
		irc.SendRaw(fmt.Sprintf("NOTICE %s :\x01TIME %s\x01", e.Name(), time.Now().UTC().Format(time.RFC1123)))
	})

	irc.AddCallback("CTCP_PING", func(e ircmsg.Message) {
		irc.SendRaw(fmt.Sprintf("NOTICE %s :\x01%s\x01", e.Name(), e.Params[1]))
	})
}
